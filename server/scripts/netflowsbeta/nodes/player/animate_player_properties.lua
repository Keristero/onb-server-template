...
return {
    ...
    override_func = function (node,context)
        ...
        Net.animate_player_properties(context.player_id, keyframes)
        async(function ()
            await(Async.sleep(math.max(total_duration-0.1,0)))--we remove a little bit of the duration to compensate for network lag if we can
            --unfortunately there is no way for us to get the timing right for all connections
