...
| animate_bot_properties | Animate bot, does not wait for the animation to end  | <pre>bot_id<br></pre>                       | <pre>first_keyframe_details<br>on_animation_duration_elapsed<br></pre> |                       |
| create_bot             | Create a bot                                         | <pre>bot<br></pre>                          | <pre>on_actor_interaction<br></pre>                                    | <pre>bot_id<br></pre> |
| animate_bot            | Animate bot, does not wait for the animation to end. | <pre>bot_id<br>state_name<br>loop<br></pre> |                                                                        |                       |
...
|         class          |                                                                description                                                                 |                                                           arguments                                                           |                                                                                         handlers                                                                                          | output |
|------------------------|--------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------|
| color_details          | describes a color                                                                                                                          | <pre>r<br>g<br>b<br>a<br></pre>                                                                                               |                                                                                                                                                                                           |        |
...
| keyframe_detail_values | used by keyframe details, you can specify hardcoded values for the keyframe, if no X,Y,Z is specified, it will use the x,y,z of this node. |                                                                                                                               | <pre>Animation<br>Animation Speed<br>X<br>Y<br>Z<br>ScaleX<br>ScaleY<br>Rotation<br>Direction<br>Sound Effect<br>Sound Effect Loop<br></pre>                                              |        |
...
| keyframe_details       |                                                                                                                                            | <pre>duration<br></pre>                                                                                                       | <pre>Animation<br>Animation Speed<br>X<br>Y<br>Z<br>ScaleX<br>ScaleY<br>Rotation<br>Direction<br>Sound Effect<br>Sound Effect Loop<br>_next_keyframe<br>_keyframe_detail_values<br></pre> |        |
...
## $${\color{#3cb44b}dialogue}$$
dialogue related nodes

|      class      |                       description                       |                                   arguments                                    |              handlers               |             output             |
|-----------------|---------------------------------------------------------|--------------------------------------------------------------------------------|-------------------------------------|--------------------------------|
| prompt_player   | Prompt player for text, _next after answer              | <pre>player_id<br>character_limit<br>default_text<br></pre>                    |                                     | <pre>prompt_answer<br></pre>   |
...
| quiz_player     | Quiz player, _next after answer                         | <pre>player_id<br>option_a<br>option_b<br>option_c<br></pre>                   | <pre>on_a<br>on_b<br>on_c<br></pre> | <pre>prompt_answer<br></pre>   |
...
| call_function_netflow     | call a function_netflow by name, it can be in any area - and there can be multiple with the same name, as an extra convenience you can define any custom properties you want on this node and they will be added to the context | <pre>function_name<br></pre>       | <pre>_on_callback<br></pre>                                                                   |        |
| map                       | map some variable names to new names, the key should be the current name, and the value should be the new name                                                                                                                  |                                    |                                                                                               |        |
...
| function_netflow_callback | if this node was reached from a call_function_netflow, this node will return all the values in its context back to the _on_callback handler of the call_function_netflow                                                        |                                    |                                                                                               |        |
...
|           class           |                       description                       |                                           arguments                                            |                                handlers                                |             output             |
|---------------------------|---------------------------------------------------------|------------------------------------------------------------------------------------------------|------------------------------------------------------------------------|--------------------------------|
| animate_player            | Animate player, does not wait for the animation to end. | <pre>player_id<br>state_name<br>loop<br></pre>                                                 |                                                                        |                                |
...
| fade_player_camera        | fade camera to color for player                         | <pre>player_id<br>color<br>durationInSeconds<br></pre>                                         |                                                                        |                                |
| move_player_camera        | move player camera                                      | <pre>player_id<br>x<br>y<br>z<br>holdTimeInSeconds<br></pre>                                   |                                                                        |                                |
| kick_player               | Kick player from server                                 | <pre>player_id<br>reason<br>warp_out<br></pre>                                                 |                                                                        |                                |
| initiate_encounter        | start an encounter for a player                         | <pre>player_id<br>package_path<br>data<br></pre>                                               |                                                                        |                                |
...
| unlock_player_camera      | unlock player camera (back to avatar?)                  | <pre>player_id<br></pre>                                                                       |                                                                        |                                |
| animate_player_properties | Animate player, does not wait for the animation to end  | <pre>player_id<br></pre>                                                                       | <pre>first_keyframe_details<br>on_animation_duration_elapsed<br></pre> |                                |
...
|     class     |                                                  description                                                   |                                                    arguments                                                     | handlers | output |
|---------------|----------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|----------|--------|
| save_variable | save variable from the context to the data store, you can save values with an alias using saved_variable_name  | <pre>unique_to_player_secret<br>unique_to_area_id<br>saved_variable_name<br>variable_name<br>table_key<br></pre> |          |        |
| load_variable | load variable from the  data store to the context, you can load values with an alias using saved_variable_name | <pre>unique_to_player_secret<br>unique_to_area_id<br>saved_variable_name<br>variable_name<br>table_key<br></pre> |          |        |
